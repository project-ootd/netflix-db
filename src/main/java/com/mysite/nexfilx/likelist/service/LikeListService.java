...
public class LikeListService {
    ...
    public boolean list(String useremail, Long contentsid){
        ...
        ProfileName profileName = profileRepository.findByUserId(user.getId());
        //좋아요 체크 상태 함수
        boolean rs = checkLike(profileName,netflixContents);
        //그에 따른 if문으로 작성.
...
//        List<NetflixContents> netflixContentsList = contentRepository.findByLikeStatus(netflixContents.getId());
        ...
        if(!rs) {
            ...
            likeList.setProfileName(profileName);
            ...
            likeList=likeListRepository.findByProfileNameIdAndNetflixContentsId(profileName.getId(),netflixContents.getId()).orElseThrow(()-> new NullPointerException());
    ...
    public boolean checkLike(ProfileName profileName, NetflixContents netflixContents){
        boolean check = likeListRepository.findByProfileNameIdAndNetflixContentsId(profileName.getId(),netflixContents.getId()).isPresent();
    ...
    public List<NetflixContents> checklist(String useremail){
        User user = userRepository.findByUseremail(useremail).orElseThrow(()-> new NullPointerException());
        ProfileName profileName = profileRepository.findByUserId(user.getId());
        List<NetflixContents> likeList = likeListRepositoryImpl.getQslUserLike(profileName.getId());
        return likeList;
    }
